...
func (conf Config) startDeviceAuth() (*deviceAuthConnect, error) {
	...
	resp, err := authHTTPClient().PostForm("https://login.live.com/oauth20_connect.srf", url.Values{
...
func (conf Config) pollDeviceAuth(deviceCode string) (t *oauth2.Token, err error) {
	resp, err := authHTTPClient().PostForm(microsoft.LiveConnectEndpoint.TokenURL, url.Values{
		"client_id":   {conf.ClientID},
